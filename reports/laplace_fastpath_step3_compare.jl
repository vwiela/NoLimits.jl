...
function run_step3_compare(; repeats::Int=2)
    ...
    for case in cases
        ...
        # Warmup both modes once to amortize compilation.
...
function write_step3_markdown(rows; path::String=joinpath(@__DIR__, "laplace_fastpath_step3_auto_vs_off_2026-03-13.md"))
    open(path, "w") do io
        println(io, "# Step 3 Fast-Path Comparison (`auto` vs `off`)")
        println(io)
        println(io, "- Date: ", STEP3_DATE)
        println(io, "- Scope: Step 3 enables scalar-inner fast backend for fully eligible models.")
...
function main()
    rows = run_step3_compare()
    out = write_step3_markdown(rows)
    println("Wrote step3 auto-vs-off report to: ", out)
    for r in rows
        println(r)
    end
end
