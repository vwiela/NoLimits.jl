...
const STEP2_DATE = "2026-03-12"

function run_step2_compare(; repeats::Int=1)
...
function write_step2_markdown(rows; path::String=joinpath(@__DIR__, "laplace_fastpath_step2_auto_vs_off_2026-03-12.md"))
    open(path, "w") do io
        println(io, "# Step 2 Fast-Path Comparison (`auto` vs `off`)")
        println(io)
        println(io, "- Date: ", STEP2_DATE)
        println(io, "- Scope: Step 2 adds strict per-outcome eligibility detection + logging only (no numerical fast-path backend).")
        println(io)
...
function main()
    rows = run_step2_compare()
    out = write_step2_markdown(rows)
    println("Wrote step2 auto-vs-off report to: ", out)
    for r in rows
        println(r)
    end
end
